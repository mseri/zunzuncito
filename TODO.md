...
The trunk boundary is not a guess. WASTE measured a 3-bit trunk: it freed over 6 GB
...
This does not make either model resident at 4 GB. It buys roughly 16–22% fewer
bytes per token and a proportional rise in slots/layer, which matters where the hit
rate is low (`gemma4 --ram 4`, `lfm25 --ram 4`) and not at all where it is already
around 90% (`lfm25 --ram 8`, which is compute-bound rather than disk-bound).
...
is still 1.84x. The 3-bit codebook is the binding constraint, not the parameter fit.
...
LUT does not improve. At 3 bits that changes: the unpack gets expensive, and the table
...
in; that is for the q3 tier only, but the file stops being uniformly GGUF-compatible.
...
capacity is not our constraint either. (This does not touch `--expert-edge`, which is
